...
const VolumeVolatility: React.FC = () => {
  ...
  const fetchVolumeVolatilityData = async (symbol: string) => {
    ...
    try {
      ...
      setError('Failed to load volume data. Using default values.');
          ...
          {isLoading ? (
            ...
            <>
              {selectedToken && tokenVolatility && (
                <>
                  <div className="flex justify-between items-center mb-1">
                    ...
                    <div className="text-[9px] text-theme-accent">{tokenVolatility.formatted}</div>
                  </div>
              ...
              {comparisonVolatility.filter(item => item.symbol !== 'Market').map((item, index) => (
                <div key={index}>
                  <div className="flex justify-between items-center mb-1">
                    ...
                    <div className="text-[9px] text-theme-accent">{item.formatted}</div>
                  </div>
                  <div className="h-1.5 bg-theme-accent/20 rounded-full overflow-hidden mb-2">
                    <div 
                      className="h-full bg-theme-accent rounded-full" 
              ...
              <div className="flex justify-between items-center mb-1">
                ...
                <div className="text-[9px] text-theme-accent">{marketAvgVolatility.toFixed(1)}%</div>
              </div>
